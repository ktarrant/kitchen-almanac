...
def test_crop_list_uses_active_catalog(catalog_client: TestClient) -> None:
    ...
    assert [crop["commodity_section_position"] for crop in payload["crops"]] == [1, 12, 26]
    assert payload["crops"][0]["commodity_section_title"] == "Asparagus"
...
def test_cultivar_catalog_exposes_overrides_inheritance_and_distinct_listings(
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["dataset_id"] == "cultivar-catalog-v1-29d8a32d94755e3d"
    assert payload["crop_dataset_id"] == "kitchen-almanac-v2-9b565d98c0dcf2fd"
...
def test_suitability_assessment_is_versioned_explainable_and_deterministic(
    ...
    assert assessment["cultivar_dataset_id"] == "cultivar-catalog-v1-29d8a32d94755e3d"
...
def test_wishlist_builder_adds_confirmed_and_custom_entries_one_at_a_time(
    ...
    assert wishlist["cultivar_dataset_id"] == "cultivar-catalog-v1-29d8a32d94755e3d"
